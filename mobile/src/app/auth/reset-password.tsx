        ...
        <Animated.View entering={FadeInDown.delay(100).springify()} className="items-center w-full">
          ...
          <View className="w-24 h-24 bg-vanz-teal/20 rounded-full items-center justify-center mb-6 relative">
            ...
            <View className="w-16 h-16 bg-vanz-teal rounded-full items-center justify-center shadow-glow-teal">
              <Mail size={32} color={colors.white} strokeWidth={2.2} />
            </View>
          </View>
          <Text className="text-content text-3xl font-black mb-4 text-center">{t('auth.resetSuccessTitle')}</Text>
      ...
      <View className="flex-1 justify-center">
        ...
        <View className="px-6 py-8 relative z-10">
          <View className="items-center mb-10 mt-4">
            ...
            <Text className="text-content text-3xl font-black mb-2 text-center">{t('auth.resetTitle')}</Text>
            <Text className="text-content-secondary text-base font-medium text-center px-4">
          ...
          <View className="mb-8 gap-5">
            <View>
              <Text className={`text-content font-extrabold mb-2 text-sm ${isRtl ? 'text-right mr-1' : 'ml-1'}`}>{t('auth.emailLabel')}</Text>
              <TextInput
                className={`bg-surface-elevated rounded-2xl border-2 shadow-sm px-5 h-16 text-lg text-content ${focusedInput === 'email' ? 'border-vanz-teal' : 'border-white'} ${isRtl ? 'text-right' : ''}`}
          ...
          >
            <LinearGradient
              colors={loading || !email ? ['#38B6FF80', '#2196D680'] : [colors.teal, colors.tealDark]}
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 0 }}
              className="w-full h-full items-center justify-center"
            >
          ...
          <View className="items-center mt-8 px-2">
            <TouchableOpacity onPress={() => router.back()} className="py-2 px-4">
              <Text className="text-content-secondary font-bold">{t('auth.cancelButton')}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
